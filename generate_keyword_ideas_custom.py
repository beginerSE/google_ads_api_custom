...
_DEFAULT_LOCATION_IDS = ["20636", "20637", "2392", "20646", "20634", "20635", "20651", "20624", "20663", "20645"]  # location ID for New York, NY(1023191)
...
def main(
    ...
    for idea in keyword_ideas:
        if count == 0:
            ...
            r = s.cse().list(q = idea.text,
                cx = CUSTOM_SEARCH_ENGINE_ID,
                lr = 'lang_ja',
                num = 10,
                start = 1).execute()
            for d in r['items']:
                base_data.append(d['link'])
            print('base',base_data)
        count += 1
        
        competition_value = idea.keyword_idea_metrics.competition.name
        #print('単価',idea.keyword_idea_metrics.average_cpc_micros,
        ...
        # 参照：https://developers.google.com/google-ads/api/reference/rpc/v14/KeywordPlanHistoricalMetrics
        #)
        cpc = idea.keyword_idea_metrics.high_top_of_page_bid_micros/1000000
        #print(idea)
        print(
            f'関連キーワード： "{idea.text}" '
            f'検索ボリューム："{idea.keyword_idea_metrics.avg_monthly_searches}" '
            f'競合性： "{competition_value}" '
            f'CPC： "{cpc}" '
            ".\n"
        )
        if count > 1 :
            ...
            r = s.cse().list(q = idea.text,
                ...
                start = 1).execute()
            for d in r['items']:
                sug_data.append(d['link'])
            print('同時ランク率',idea.text, len(set(base_data) & set(sug_data))*10 ,'%')
        print('-------------------------------------------------------------------')
